...
The project pins the four Pi devDependencies to exactly `0.86.0`, and `npm run check` verifies the code against those local dependencies. For a version-matched interactive run, start `node node_modules/@earendil-works/pi-coding-agent/dist/bundle/cli.js`; if you use the global `pi` CLI instead, install the same `0.86.0` and start a new process. `/reload` reloads the extension inside the currently running process and does not upgrade that runtime, so an older global install never switches an existing session. Rail does not auto-upgrade the global CLI.
...
The tests use local mock providers and the repository's Pi bundle, not paid model calls. See [the shared Pi 0.86.0 migration report](docs/pi-0.86.0-migration.md) and [the Team integration report](docs/subagent-team-pi-0.86.0.md) for their respective scope and verification. The Team report also covers cancellation of parked coordinators during shutdown/deletion and queued model changes.
...
Toggles Pi 0.86.0's native OpenAI-compatible priority service tier for the current model:
