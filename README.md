...
- Automatically delivered cooperation data is retained through native Pi custom messages, not just one temporary context transform. Later turns or compaction can recover missing data for the current binding, within a 64-delivery / 1 MiB window including its compact roster, without scheduling extra model turns. Historical snapshots describe their recorded sequence; use the latest sequence and authoritative control replies for state. Redirect does not clear pause: explicit resume is still required.
- A's final summary is generated only after every worker has a native terminal outcome, including failures—not merely a `report` or `finish` claim. If A explicitly received the worker barrier and a subsequent native context checkpoint, its final answer completes the run without a second summary request. If A settles early, its parent call stays pending and one final-summary continuation receives the complete worker result snapshot. An empty worker answer without a structured result fails that worker; a structured `succeeded`/`partial`/`blocked`/`failed` task outcome is distinct from native run completion. Redirect invalidates a prior deliverable candidate. Worker failure normally allows others to finish; coordinator failure or cancellation stops the team and may prevent a successful summary.
