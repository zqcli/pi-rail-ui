...
- `fastMode: true` enables Pi's native priority service tier for one stateless GPT dispatch or records that policy when creating/adopting a persistent GPT agent. It is off by default and is not inherited from the parent Pi session. Existing persistent targets use their saved policy; change it only through `/rail-agent`. Unsupported models retain the policy as **FAST inactive** without receiving `service_tier`. Grouped and control calls cannot change Fast policy.
- Every stateless dispatch and every persistent RPC worker starts the standalone native-search extension pinned to `live` (`-e <rail-oai-search-standalone> --rail-oai-search-mode live`) instead of inheriting the parent session's `/rail-oai-search` selection; GPT eligibility still decides whether a child request actually receives the hosted `web_search` tool. The parent dispatch header always ends with `ContextWindow ... · FAST ... · SEARCH on`, each grouped child panel repeats that fixed policy after its own `ContextWindow`/`FAST` values, and single result panels plus control calls do not repeat it. Search policy is display-only: it never enters the Tool parameters schema or result details.
...
Rail Subagents use the same native mechanism through `fastMode: true`. Subagent Fast is deliberately GPT-only, applies before the child model's first provider request, and is a dispatch/persistent-agent policy rather than an implicit inheritance of the parent's current command state.

It is active for Pi's documented `samplingParams` request paths: `openai-completions`, `openai-responses`, and `azure-openai-responses`. `openai-codex-responses` remains inactive because Pi 0.85.1's Codex builder uses a separate `serviceTier` option and does not forward model `samplingParams`.
