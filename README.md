...
A team runs with **two `subagent_team` calls in consecutive messages**. First `prepare` the complete plan: each member is an object with `alias` and `task`, plus optional `model`, `fastMode` and `cwd` (`null` means the default). Do not supply `teamId`: prepare generates it. Put shared scope in `brief`; keep personal restrictions separate from another member's authorization:

```json
{"action":"prepare","coordinator":{"alias":"A","task":"Coordinate B1/B2's read-only review. Await all worker outcomes with team finish, then summarize findings and failures."},"workers":[{"alias":"B1","task":"Review implementation correctness without edits. Report findings to A.","model":"cus-resp/gpt-6-luna:max","fastMode":true},{"alias":"B2","task":"Review test coverage without edits. Report findings to A.","model":null,"fastMode":null,"cwd":null}],"brief":{"goal":"Review implementation and tests","target":"the current repository","acceptanceCriteria":["Return findings with file locations"],"constraints":["No production changes"],"authorizations":[{"member":"A","allowed":["Coordinate and summarize"],"forbidden":["Edit files"]},{"member":"B1","allowed":["Read implementation"]},{"member":"B2","allowed":["Run offline tests"]}]}}
```

prepare starts nothing. It validates every member (model, Fast eligibility, cwd, and that each alias is new) and prints the resolved plan. A rejected prepare names the member and field; fix it and prepare again. The resolved model and cwd are pinned, so a later parent model switch does not change what launches. Then, in the next message, launch with only the returned id:

```json
{"action":"launch","teamId":"<teamId>"}
```

launch starts the coordinator and every worker from the host in one Tool Call panel and returns when the coordinator has finished with all worker outcomes. Its other fields must be empty or `null`: the plan was fixed at prepare, so cancel and prepare again to change it. Do not start team members with the `subagent` tool; a planned team rejects that. (For compatibility, `coordinator`/`workers` given as alias strings keep the older flow of two paired `subagent` calls carrying the `teamId`.)

Every child's first context includes the brief and all members' bounded assignments, cwd and effective model/Fast/Search policy. The hosting parent is not member A and cannot answer live messages while its launch call is pending. A's personal read-only restriction does not revoke B2's separately scoped test permission. A brief is context, not an OS sandbox or an additional privilege grant; do not put secrets in shared scope.

Children use the `team` tool; sender and team are supplied by the runtime, not arguments:

| Action | Example arguments / meaning |
| --- | --- |
| `send` | `{"action":"send","to":"B2","message":"Please check the error path."}` — queue a message to a teammate, not yourself. Optional `replyTo` and `supersedes` reference known message IDs; only your own prior message may be superseded. |
| `report` | `{"action":"report","to":null,"message":"Blocked; need scope clarification.","wait":{"kind":"message"}}` — report to A and atomically wait for a reply; omit `wait` to report without waiting. Omit/null `to` by default, or explicitly use the current coordinator's actual alias; another recipient is rejected. |
| `wait` | `{"action":"wait","wait":{"kind":"member","member":"B2"}}` — await a member's terminal outcome. Use `{"kind":"message","from":"B2"}` to receive B2's send/report events without discarding other senders' messages; omit `from` for the whole inbox. For A only, use `{"kind":"workers"}` for all worker outcomes. |
| `control` | A only: `{"action":"control","to":"B1","command":"pause"}`; `resume` clears the pause. `redirect` also requires `message` and replaces a pending wait with the new direction, but does not clear an explicit pause or undo work. `cancel` (optional reason `message`) stops that one worker with a `cancelled` outcome without cancelling the team. |
| `finish` | A uses `{"action":"finish"}` without message/result to await workers, then writes its summary. A worker may use `{"action":"finish","message":"Checked the error path"}` or `{"action":"finish","result":{"status":"partial","summary":"Checked the error path","limitations":["No live system test"]}}`. This records a deliverable candidate, not a native terminal state. |

**`wait`, `report` with `wait`, and `finish` must each be the sole tool call in their assistant batch**, never alongside another tool. Use event-driven waits, not polling. Self-waits, unknown members and dependency cycles are rejected, and A cannot `finish` or wait for all workers while a worker is paused: resume or cancel it first. A report argument error means the report was not sent: correct the fields and retry it instead of skipping a required report and going directly to `wait`, which can leave the coordinator waiting for your missing report.

- If every live member is parked on something nobody can provide (for example mutual sender-filtered waits), the Hub notices after a short grace and gives A one `team_stalled` error that bypasses any sender filter. If A waits again without anything changing, the team fails with that reason instead of idling until the deadline. When a recipient ends with unread queued messages, their senders and A receive an `undelivered` notice.

- Scheduling allows **four active worker permits plus independent A admission**. Waiting/paused workers release permits but retain their sessions; neither cooperative waiting nor pausing ends the enclosing parent Tool Call. A running member first shows `PAUSE REQUESTED`, then `PAUSED` at a safe point; in-flight model/tool/compaction work may finish. This is not an immediate process freeze or rollback. Redirected directions enter the model at its next native context gate. The latest unobserved direction has a reserved delivery slot even under inbox pressure; multiple pending redirects coalesce to the latest direction. Tool calls generated under an old instruction revision are blocked at the next preflight so the native loop can replan, rather than rewriting their arguments or aborting the whole team. Work already admitted or started cannot be rolled back.
- Automatically delivered cooperation data is retained through native Pi custom messages, not just one temporary context transform. Native compaction owns summarized history: afterwards only the compact roster (brief and assignments) and deliveries the native context should still hold are restored for the current binding, within a 64-delivery / 1 MiB window, without scheduling extra model turns. Wait and control replies carry a compact current status (states, revisions, result/error previews); complete outcomes arrive only for the awaited member or, at A's barrier, for all workers. Historical snapshots describe their recorded sequence; use the latest sequence and authoritative control replies for state. Redirect does not clear pause: explicit resume is still required.
- A's final summary is generated only after every worker has a native terminal outcome, including failures—not merely a `report` or `finish` claim. If A explicitly received the worker barrier and a subsequent native context checkpoint, its final answer completes the run without a second summary request. After the barrier, extra `send`/`report`/`control`/other waits are rejected as correctable tool errors, and a repeated barrier returns the same outcomes. If A settles early, its parent call stays pending and one final-summary continuation receives the complete worker outcomes; if the team is stalled at that point, A first gets one turn to unblock it. An empty worker answer without a structured result fails that worker; a structured `succeeded`/`partial`/`blocked`/`failed` task outcome is distinct from native run completion. Redirect invalidates a prior deliverable candidate; a redirect that arrives while a worker writes its final answer gives that worker one continuation instead of failing it. Worker failure normally allows others to finish; coordinator failure or cancellation stops the team and may prevent a successful summary.
- The team deadline defaults to **one hour from prepare** (`timeoutSeconds`, positive, at most `86400`). Use `null` or omit it unless a deadline was requested; do not invent 120/180-second limits for reviews or high-thinking models. This budget covers startup, reasoning, tools, waiting and the final summary, not a single tool call. Explicit short deadlines remain honored. launch admits all members at once; with the legacy paired calls, all members must join within **30 seconds of the first join**. Failed dispatch results and status preserve the actual cancellation reason instead of reporting only an RPC shutdown. Inspect with `subagent_team` arguments `{"action":"status","teamId":"<teamId>"}` (omit id to list), or cancel with `{"action":"cancel","teamId":"<teamId>","reason":"Stop this review"}`. Aborting the launch call (or, in the legacy flow, either parent dispatch) cancels the team. Reload marks unfinished history `interrupted`; it does not restore pending Promises or automatically resume work. After a failure, prepare a new team: members that started keep their persistent aliases and sessions for inspection, so give them new aliases; members that never passed a team gate are removed so their aliases can be reused.
- New public events include runtime-authenticated `from`, `to`, `messageId`, `timestamp` and `seq`; system state/results come from `@hub` with `member` identifying the subject. Reply routing includes `requestId`. A `queued` receipt means inbox acceptance, not that a model read it; control `applied` means Hub state changed, not that work was immediately stopped. `observedRevision` records context delivery, not model comprehension. Old history remains readable without fabricated routing metadata.
- Status appears in existing Tool Call output/panels, not a new standalone GUI overlay. Single-team status shows bounded assignment/result previews and recent `from -> to` routes; listing all teams returns summaries, not 32 copies of full briefs/results. Complete data remains in native result details; the parent session journals only milestones (admission, member outcomes, phases) with the last 16 events. Inbox/history and summary snapshots are bounded: a parent session keeps at most 32 teams (the oldest finished team is dropped for a new one; 32 active teams are rejected), messages are limited to 8 KiB, inbox overflow is rejected, and old event history may be dropped. Damaged or excess history entries are skipped on reload rather than disabling subagents. Summary snapshots retain at most 16 KiB of each worker's output with an explicit truncation marker. Shared briefs have a 32 KiB serialized JSON cap; assignments have a 16 KiB aggregate cap (task text at most 8 KiB); structured results have a 12 KiB aggregate cap, including optional findings/evidence/limitations/artifacts. Over-limit input is rejected before storage, not silently truncated. Put large artifacts in files and send paths plus concise findings rather than relying on messages or summaries as full transcripts. See [Team collaboration hardening](docs/subagent-team-hardening.md) for message examples and validation scope, and [Team review fixes](docs/subagent-team-review-fixes.md) for liveness, context-cost and journal changes.
...
The tests use local mock providers and the repository's Pi bundle, not paid model calls. See [Pi 0.87.1 compatibility](docs/pi-0.87.1-compatibility.md) and [Pi 0.87.0 migration](docs/pi-0.87.0-migration.md) for the shared runtime. See the [Team Pi 0.87.1 integration report](docs/subagent-team-pi-0.87.1.md) for current Team-specific verification. The [Team Pi 0.86.0 report](docs/subagent-team-pi-0.86.0.md) remains a historical record.
