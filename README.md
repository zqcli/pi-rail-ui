...
Pi Rail UI's development and full-suite baseline is Pi `0.87.0`; see the [startup fix and verification scope](docs/pi-ai-extension-loading.md). Earlier Pi releases are not supported.
...
The project pins the four Pi devDependencies to exactly `0.87.0`, and `npm run check` verifies the code against those local dependencies. For a version-matched interactive run, start `node node_modules/@earendil-works/pi-coding-agent/dist/bundle/cli.js`; if you use the global `pi` CLI instead, install the same `0.87.0` and start a new process. `/reload` reloads the extension inside the currently running process and does not upgrade that runtime, so an older global install never switches an existing session. Rail does not auto-upgrade the global CLI.
...
The tests use local mock providers and the repository's Pi bundle, not paid model calls. See `docs/pi-0.87.0-migration.md` for the migration scope and verification report.
...
Toggles Pi 0.87.0's native OpenAI-compatible priority service tier for the current model:
