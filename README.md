...
First call `subagent_team` to prepare. Do not supply `teamId`: prepare generates it and now rejects a non-empty caller-supplied id. Put shared scope in `brief`; keep personal restrictions separate from another member's authorization:

```json
{"action":"prepare","coordinator":"A","workers":["B1","B2"],"brief":{"goal":"Review implementation and tests","target":"the current repository","acceptanceCriteria":["Return findings with file locations"],"constraints":["No production changes"],"authorizations":[{"member":"A","allowed":["Coordinate and summarize"],"forbidden":["Edit files"]},{"member":"B1","allowed":["Read implementation"]},{"member":"B2","allowed":["Run offline tests"]}]}}
```

Every child's first context includes the brief and all members' bounded assignments, cwd and effective model/Fast/Search policy. The hosting parent is not member A and cannot answer live messages while its outer dispatch is pending. A's personal read-only restriction does not revoke B2's separately scoped test permission. A brief is context, not an OS sandbox or an additional privilege grant; do not put secrets in shared scope.

Then replace `<teamId>` with the returned id and emit the following as **two sibling `subagent` tool calls in the same assistant turn**, not sequential calls and not one combined `tasks` array. A is single; all prepared Bs belong in the grouped call. `teamId` is top-level in both. These examples omit `model` to use the current Pi model; choose fresh aliases for each team. Native parent sessions check that both sides are present in the same assistant message before starting members. If an unpaired call is rejected, reuse that prepared teamId and resend BOTH sides in the next message, not just the missing side. The prepare result prints both calls with the real teamId and aliases; a single worker still goes in a `tasks` array. Omit `target`, `session`, `control` and `chain` or set them to `null` (`session`/`control` accept `null`) rather than filling placeholders: a non-empty `control.message` turns the call into control mode. A rejected launch names the offending field and repeats the correct call shape.
