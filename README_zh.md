...
- 每次 stateless dispatch 与每个 persistent RPC worker 都会通过 `-e <rail-oai-search-standalone> --rail-oai-search-mode live` 以 `live` 启动 standalone 原生搜索扩展，不继承 parent session 的 `/rail-oai-search` 选择；child 请求是否真正注入 hosted `web_search` 仍由 GPT 资格判断。父级 dispatch 头部固定以 `ContextWindow ... · FAST ... · SEARCH on` 结尾，grouped 中每个 child 面板都会在自己的 `ContextWindow`/`FAST` 之后重复该固定 policy，single result 面板与 control 调用不会重复显示。Search policy 仅用于显示，不会进入 Tool parameters schema 或 result details。
...
该模式是 session-local 的。Rail child 进程不会继承 parent 的 slash 状态：每次 stateless dispatch 与每个 persistent RPC worker 都会通过 `--rail-oai-search-mode live` 启动 standalone 搜索扩展，因此即使 parent 处于 `cached`、`off` 或 probe 状态，child 也始终从 `live` 开始。child 的 dispatch 头部和 grouped child 面板会在 `ContextWindow`/`FAST` 之后固定显示 `SEARCH on`；single result 面板与 control 调用不会显示。上面的 GPT 资格判断仍决定是否真正注入 hosted tool；该 policy 仅用于显示，不会给 Tool parameters 或 details 增加字段。

对于可安全包装的 `openai-responses` 和 `azure-openai-responses` SSE provider，Rail 还会旁路观察真实 hosted `web_search_call` 流，并在对应 Assistant turn 内插入一个搜索活动区。只有观察到真实 hosted call 后才显示；搜索中自动展开，成功后自动折叠，并以有界列表展示动作和来源链接；可单击或按 `Ctrl+O` 展开/折叠。已 finalize 的 snapshot（completed、failed 或 cancelled）作为 session custom entry 保存，不会进入 LLM context。每份已 finalize 的 snapshot 同时会以 Pi `entry_appended` 事件转发，因此 parent subagent 的 usage 行可以统计 Rail 观测到的 `web_search_call` 次数；仅用于显示的 `SEARCH on` policy 与回答中的 citation 都不会计入该次数。Rail 不覆盖 native extension provider，也不改变 Codex 默认 WebSocket transport；这些路径仍可使用 hosted search，但只显示最终回答。
