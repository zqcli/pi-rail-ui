...
Pi Rail UI 的开发和完整测试基线为 Pi `0.87.0`，详见[启动修复与验证范围](docs/pi-ai-extension-loading.md)。不再支持更早版本的 Pi。
...
- 显式 context budget 按实际 child model 和 cwd 解析 Pi 0.87.0 的 `compaction.modelOverrides["provider/modelId"]`。父端预检遵循已保存／默认的非交互 project trust，child helper 再按实际 session trust 校验；不会隐式批准项目设置，省略 budget 时不会安装 override。
...
项目把四个 Pi devDependencies 精确固定在 `0.87.0`，`npm run check` 针对这些本地依赖校验代码。需要版本匹配的交互启动时，运行 `node node_modules/@earendil-works/pi-coding-agent/dist/bundle/cli.js`；若改用全局 `pi` CLI，请安装相同 `0.87.0` 并启动新进程。`/reload` 只在当前运行进程内重载扩展，不会升级当前 runtime，因此旧全局安装永远不会切换既有会话。Rail 不会自动升级全局 CLI。
...
测试使用本地 mock provider 和仓库内的 Pi bundle，不调用付费模型。迁移范围和验证报告见 `docs/pi-0.87.0-migration.md`。
...
切换当前模型的 Pi 0.87.0 原生 OpenAI-compatible priority service tier：
