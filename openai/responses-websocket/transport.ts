...
// A busy cache slot can open a short-lived second socket for a concurrent
// request. Keep every session-owned socket here so shutdown closes both the
// cached winner and any in-flight overflow slot.
...
function trackSessionSocket(sessionId: string, socket: WebSocket): () => void {
	...
	const untrack = () => {
		if (!tracked) return;
		tracked = false;
		socket.off("close", untrack);
		const current = sessionSockets.get(sessionId);
		current?.delete(socket);
		if (current?.size === 0) sessionSockets.delete(sessionId);
	};
...
export function closeRailResponsesWebSocketSessions(sessionId?: string): void {
	const addEntries = (sockets: Set<WebSocket>, entries: Map<string, ConnectionEntry> | undefined) => {
	...
	if (sessionId) {
		...
		const sockets = new Set<WebSocket>(sessionSockets.get(sessionId) ?? []);
		addEntries(sockets, connectionCache.get(sessionId));
		for (const socket of sockets) closeSilently(socket, 1000, "session_cleanup");
	...
	for (const trackedSockets of sessionSockets.values()) for (const socket of trackedSockets) sockets.add(socket);
	for (const socket of sockets) closeSilently(socket, 1000, "session_cleanup");
...
async function acquireConnection(options: ResponsesWebSocketRequestOptions): Promise<AcquiredConnection> {
	...
	if (!options.sessionId) {
		...
		return { ...connected, reused: false, isCurrent: () => true, release: () => closeSilently(connected.socket) };
	}
	const generation = generationFor(options.sessionId);
	const isCurrent = () => generation.active;
	...
	if (cached) {
		...
		if (!cached.busy && Date.now() - cached.createdAt >= CONNECTION_MAX_AGE_MS) {
			...
			return {
				...
				isCurrent,
				release: ({ keep } = {}) => {
					if (!keep || !isCurrent() || !isReusable(cached.socket)) {
						cached.untrack?.();
	...
	const untrack = trackSessionSocket(options.sessionId, connected.socket);
	...
	if (entries?.has(identity)) {
		return {
			...
			release: () => {
				untrack();
				closeSilently(connected.socket);
			},
		};
	}
	const entry: ConnectionEntry = { ...connected, busy: true, createdAt: Date.now(), idleTimer: undefined, continuation: undefined, untrack };
	entries = connectionCache.get(options.sessionId);
	if (!entries) {
		entries = new Map();
	...
	return {
		...
		isCurrent,
		release: ({ keep } = {}) => {
			if (!keep || !isCurrent() || !isReusable(entry.socket)) {
				entry.untrack?.();
...
export async function runResponsesWebSocketRequest(
	...
	if (options.signal?.aborted || !acquired.isCurrent()) {
		acquired.release({ keep: false });
		throw new Error(options.signal?.aborted ? "Request was aborted" : "Responses WebSocket session was cleaned up");
	}
