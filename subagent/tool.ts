...
const SessionSourceSchema = Type.Object({
	mode: StringEnum(["new", "fork", "exclusive"] as const, {
		description: "new creates a child session; fork copies an existing saved session; exclusive opens it in place",
	}),
	path: Type.Optional(Type.String({ description: "Existing Pi session path for fork or exclusive mode" })),
});

const TaskItem = Type.Object({
	agent: Type.Optional(Type.String({ description: "Agent profile to create" })),
...
const StatefulSubagentParams = Type.Object({
	agent: Type.Optional(Type.String({ description: "Agent profile to create (single mode)" })),
...
type TaskParams = {
	...
	task: string;
	cwd?: string;
	session?: { mode: "new" | "fork" | "exclusive"; path?: string };
};
...
export function installStatefulSubagentTool(pi: ExtensionAPI, options: StatefulSubagentToolOptions): void {
	pi.registerTool({
		name: "subagent",
		label: "Subagent",
		description: "Create persistent specialized subagents or continue an existing instance by alias/agentId. Supports single, parallel, and chain modes. Use agent+alias for a new persistent instance; use target for the same subagent's later tasks.",
		promptSnippet: "Create or continue persistent specialized subagents with isolated child sessions",
		promptGuidelines: [
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (projectProfiles.length > 0 && (params.confirmProjectAgents ?? true) && ctx.hasUI) {
			...
			const sessionAttachments = requestedItems.filter((item) => item.session?.mode === "fork" || item.session?.mode === "exclusive");
			for (const item of sessionAttachments) {
				if (!item.session?.path) throw new Error(`${item.session?.mode} session mode requires a path`);
			}
		...
		renderCall(args, theme) {
			const mode = args.chain?.length ? `chain (${args.chain.length})` : args.tasks?.length ? `parallel (${args.tasks.length})` : args.target ? `continue ${args.target}` : `new ${args.agent ?? "..."}`;
		...
		renderResult(result, { expanded }, theme) {
			...
			for (const item of details.results) {
				...
				lines.push(`${icon} ${theme.fg("toolTitle", theme.bold(item.alias))}${item.agentId ? theme.fg("dim", ` ${item.agentId}`) : ""}`);
