...
function nativeRepairPreparation(
	...
	const projectedIndexes = new Map(projectedBranch.map((entry, index) => [entry.id, index]));
	let projectedAdmissionStart: number | undefined;
	for (const entry of branch.slice(rawAdmissionStart)) {
		const index = projectedIndexes.get(entry.id);
		if (index !== undefined) {
			projectedAdmissionStart = index;
			break;
		}
	}
	if (railIndex >= 0 && projectedAdmissionStart === undefined) return undefined;
	const cut = findCutPoint(projectedBranch, projectedAdmissionStart ?? 0, projectedBranch.length, settings.keepRecentTokens);
